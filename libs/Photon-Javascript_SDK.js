        ...
        const Logger = /** @class */ (function () {
            ...
            */
            ...
            Logger.prototype.format0 = function (msg, optionalParams) {
                return `${(this.prefix == '' ? '' : `${this.prefix} `) + msg} ${optionalParams.map((x) => {
                    if (x !== undefined) {
                        switch (typeof x) {
                        case 'object':
                            try {
    ...
    const PhotonPeer = /** @class */ (function () {
        ...
        */
        PhotonPeer.prototype.connect = function (appid) {
            ...
            const url = `${this.url}/${appid}?libversion=4.1.0.0`;
            if (this.subprotocol == '') {
            ...
            this._socket.onclose = function (ev) {
                ...
                if (_this._isConnecting) {
                    ...
                    if (ev.code == 1006) { // TODO: avoid using constants. what is the 1006
                        _this._onTimeout();
                    }
                    _this._onDisconnect();
                }
        ...
        */
        ...
        PhotonPeer.prototype._decode = function (data) {
            const messages = []; let number; let n; let
                newdata = data;
        ...
        PhotonPeer.prototype._onMessageReceived = function (message) {
            if (typeof message === 'object') {
                ...
                const msgJSON = {
                    err: message.err, msg: message.msg, vals: message.vals, res: message.res, evt: message.evt, irs: message.irs,
                };
                ...
                } else if (msgJSON.evt !== undefined) {
                    var code = parseInt(msgJSON.evt);
                    this._parseEvent(code, msgJSON);
                } else if (msgJSON.irs !== undefined) {
        ...
        PhotonPeer.prototype._parseMessageValuesArrayToJSON = function (vals) {
            ...
            if (Exitgames.Common.Util.isArray(vals)) {
                if (vals.length % 2 == 0) {
                    const toParse = vals; let key; let
                        value;
        ...
        PhotonPeer.prototype._removeListener = function (listeners, code, callback) {
            if ((code in listeners)) {
                ...
                listeners[code] = listeners[code].filter((x) => x != callback);
    ...
    (function (LoadBalancing) {
        ...
        const Actor = /** @class */ (function () {
            ...
            */
            Actor.prototype.setName = function (name) {
                if (this.name != name) {
                    if (this.setProp(LoadBalancing.Constants.ActorProperties.PlayerName, name, this.name)) { this.name = name; }
                }
            };
            // properties methods
            /**
        ...
        const RoomInfo = /** @class */ (function () {
            ...
            * */
            RoomInfo.prototype.getPropsListedInLobby = function () {
                const p = [];
                for (const k in this._propsListedInLobby) {
        ...
        const Room = /** @class */ (function (_super) {
            ...
            */
            Room.prototype.setIsVisible = function (isVisible) {
                if (this.isVisible != isVisible) {
                    if (this.setProp(LoadBalancing.Constants.GameProperties.IsVisible, isVisible, this.isVisible)) { this.isVisible = isVisible; }
                }
            };
            /**
            ...
            */
            Room.prototype.setIsOpen = function (isOpen) {
                if (this.isOpen != isOpen) {
                    if (this.setProp(LoadBalancing.Constants.GameProperties.IsOpen, isOpen, this.isOpen)) { this.isOpen = isOpen; }
                }
            };
            /**
            ...
            */
            Room.prototype.setMaxPlayers = function (maxPlayers) {
                if (this.maxPlayers != maxPlayers) {
                    if (this.setProp(LoadBalancing.Constants.GameProperties.MaxPlayers, maxPlayers, this.maxPlayers)) { this.maxPlayers = maxPlayers; }
                }
            };
            /**
            ...
            */
            Room.prototype.setEmptyRoomLiveTime = function (emptyRoomLiveTime) {
                if (this.emptyRoomLiveTime != emptyRoomLiveTime) {
                    if (this.setProp(LoadBalancing.Constants.GameProperties.EmptyRoomTtl, emptyRoomLiveTime, this.emptyRoomLiveTime)) { this.emptyRoomLiveTime = emptyRoomLiveTime; }
                }
            };
            /**
            ...
            */
            Room.prototype.setSuspendedPlayerLiveTime = function (suspendedPlayerLiveTime) {
                if (this.suspendedPlayerLiveTime != suspendedPlayerLiveTime) {
                    if (this.setProp(LoadBalancing.Constants.GameProperties.PlayerTtl, suspendedPlayerLiveTime, this.suspendedPlayerLiveTime)) { this.suspendedPlayerLiveTime = suspendedPlayerLiveTime; }
                }
            };
            /**
            ...
            */
            Room.prototype.setExpectedUsers = function (props) {
                if (this.setProp(LoadBalancing.Constants.GameProperties.ExpectedUsers, props, this.expectedUsers)) { this.expectedUsers = props; }
            };
            /**
            ...
            */
            Room.prototype.clearExpectedUsers = function () {
                const empty = [];
                if (this.setProp(LoadBalancing.Constants.GameProperties.ExpectedUsers, empty, this.expectedUsers)) { this.expectedUsers = empty; }
            };
            /**
            ...
            */
            Room.prototype.setMasterClient = function (actorNr) {
                if (this.setProp(LoadBalancing.Constants.GameProperties.MasterClientId, actorNr, this.masterClientId)) { this.masterClientId = actorNr; }
            };
        ...
        const LoadBalancingClient = /** @class */ (function () {
            ...
            */
            ...
            LoadBalancingClient.prototype.fillCreateRoomOptions = function (op, options) {
                ...
                if (options.plugins !== undefined) { op.push(LoadBalancing.Constants.ParameterCode.Plugins, Photon.TypeExt.String(options.plugins)); }
                ...
                if (options.expectedUsers) { op.push(LoadBalancing.Constants.ParameterCode.Add, options.expectedUsers); }
            };
            /**
            ...
            * */
            LoadBalancingClient.prototype.connectToRegionMaster = function (region) {
                ...
                if (this.connectToNameServer({ region })) {
                    return true;
                }
            ...
            */
            ...
            LoadBalancingClient.prototype.clearRooms = function () { this.roomInfos = []; this.roomInfosDict = {}; };
            ...
            LoadBalancingClient.prototype.removeActor = function (actorNr) {
                ...
                if (this.lowestActorId == actorNr) {
                    if (this.actorsArray.length > 0) { this.lowestActorId = this.actorsArray.reduce((prev, curr) => (prev.actorNr < curr.actorNr ? prev : curr)).actorNr; } else { this.lowestActorId = 0; }
                }
            };
            ...
            LoadBalancingClient.prototype.initNameServerPeer = function (np) {
                ...
                // errors
                np.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.error, () => {
                ...
                np.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.connectFailed, () => {
                    _this.changeState(LoadBalancingClient.State.Error);
                ...
                np.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.timeout, () => {
                    _this.changeState(LoadBalancingClient.State.Error);
                    _this._onErrorInternal(LoadBalancingClient.PeerErrorCode.NameServerTimeout, 'NameServer peer timeout');
                });
                np.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.connecting, () => {
                });
                np.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.connect, () => {
                ...
                np.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.disconnect, () => {
                    if (np == _this.nameServerPeer) { // skip delayed disconnect response
                        _this._cleanupNameServerPeerData();
                        np._logger.info('Disconnected');
                    }
                });
                np.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.connectClosed, () => {
                ...
                np.addResponseListener(LoadBalancing.Constants.OperationCode.GetRegions, (data) => {
                ...
                np.addResponseListener(LoadBalancing.Constants.OperationCode.Authenticate, (data) => {
            ...
            LoadBalancingClient.prototype.initMasterPeer = function (mp) {
                ...
                // errors
                mp.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.error, () => {
                    _this.changeState(LoadBalancingClient.State.Error);
                    _this._onErrorInternal(LoadBalancingClient.PeerErrorCode.MasterError, 'Master peer error');
                });
                mp.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.connectFailed, () => {
                    _this.changeState(LoadBalancingClient.State.Error);
                ...
                mp.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.timeout, () => {
                    _this.changeState(LoadBalancingClient.State.Error);
                    _this._onErrorInternal(LoadBalancingClient.PeerErrorCode.MasterTimeout, 'Master peer error timeout');
                });
                mp.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.connecting, () => {
                });
                // status
                mp.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.connect, () => {
                ...
                mp.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.disconnect, () => {
                    if (mp == _this.masterPeer) { // skip delayed disconnect response
                        _this._cleanupMasterPeerData();
                        mp._logger.info('Disconnected');
                    }
                });
                mp.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.connectClosed, () => {
                ...
                // events
                mp.addEventListener(LoadBalancing.Constants.EventCode.GameList, (data) => {
                ...
                mp.addEventListener(LoadBalancing.Constants.EventCode.GameListUpdate, (data) => {
                    const gameList = data.vals[LoadBalancing.Constants.ParameterCode.GameList];
                    const roomsUpdated = [];
                    const roomsAdded = [];
                    const roomsRemoved = [];
                    for (var g in gameList) {
                        const exist = _this.roomInfos.filter((x) => x.name == g);
                        if (exist.length > 0) {
                            const r = exist[0];
                            r._updateFromProps(gameList[g]);
                            if (r.removed) {
                ...
                // responses - check operation result: data.errCode
                mp.addResponseListener(LoadBalancing.Constants.OperationCode.Authenticate, (data) => {
                ...
                mp.addResponseListener(LoadBalancing.Constants.OperationCode.JoinLobby, (data) => {
                ...
                mp.addResponseListener(LoadBalancing.Constants.OperationCode.CreateGame, (data) => {
                ...
                mp.addResponseListener(LoadBalancing.Constants.OperationCode.JoinGame, (data) => {
                ...
                mp.addResponseListener(LoadBalancing.Constants.OperationCode.JoinRandomGame, (data) => {
                ...
                mp.addResponseListener(LoadBalancing.Constants.OperationCode.FindFriends, (data) => {
                ...
                mp.addResponseListener(LoadBalancing.Constants.OperationCode.LobbyStats, (data) => {
                    ...
                    if (!data.errCode) {
                        ...
                        if (names) {
                            for (var i = 0; i < names.length; ++i) {
                                res[i] = {
                                    lobbyName: names[i], lobbyType: types[i], peerCount: peers[i], gameCount: games[i],
                                };
                            }
                        } else {
                            for (var i = 0; i < _this.lobbyStatsRequestList.length; ++i) {
                                ...
                                res[i] = {
                                    lobbyName: l[0], lobbyType: l[1], peerCount: peers[i], gameCount: games[i],
                                };
                            }
                        }
                    } else {
                ...
                mp.addEventListener(LoadBalancing.Constants.EventCode.LobbyStats, (data) => {
                ...
                mp.addEventListener(LoadBalancing.Constants.EventCode.AppStats, (data) => {
            ...
            LoadBalancingClient.prototype.initGamePeer = function (gp, masterOpCode) {
                ...
                // errors
                gp.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.error, () => {
                    _this.changeState(LoadBalancingClient.State.Error);
                    _this._onErrorInternal(LoadBalancingClient.PeerErrorCode.GameError, 'Game peer error');
                });
                gp.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.connectFailed, () => {
                    _this.changeState(LoadBalancingClient.State.Error);
                ...
                gp.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.timeout, () => {
                    _this.changeState(LoadBalancingClient.State.Error);
                    _this._onErrorInternal(LoadBalancingClient.PeerErrorCode.GameTimeout, 'Game peer timeout');
                });
                // status
                gp.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.connect, () => {
                ...
                gp.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.disconnect, () => {
                    if (gp == _this.gamePeer) { // skip delayed disconnect response
                        _this._cleanupGamePeerData();
                        gp._logger.info('Disconnected');
                    }
                });
                gp.addPeerStatusListener(Photon.PhotonPeer.StatusCodes.connectClosed, () => {
                ...
                // responses
                gp.addResponseListener(LoadBalancing.Constants.OperationCode.Authenticate, (data) => {
                ...
                gp.addResponseListener(LoadBalancing.Constants.OperationCode.CreateGame, (data) => {
                ...
                gp.addResponseListener(LoadBalancing.Constants.OperationCode.JoinGame, (data) => {
                ...
                gp.addResponseListener(LoadBalancing.Constants.OperationCode.SetProperties, (data) => {
                ...
                gp.addResponseListener(LoadBalancing.Constants.OperationCode.Leave, (data) => {
                ...
                // events
                gp.addEventListener(LoadBalancing.Constants.EventCode.Join, (data) => {
                ...
                gp.addEventListener(LoadBalancing.Constants.EventCode.Leave, (data) => {
                ...
                gp.addEventListener(LoadBalancing.Constants.EventCode.Disconnect, (data) => {
                ...
                gp.addEventListener(LoadBalancing.Constants.EventCode.PropertiesChanged, (data) => {
                ...
                gp.addEventListener(LoadBalancing.Constants.EventCode.AuthEvent, (data) => {
                    gp._logger.debug('ev AuthEvent', data);
                    _this.connectOptions.userAuthSecret = data.vals[LoadBalancing.Constants.ParameterCode.Secret];
                });
                gp.addEventListener(LoadBalancing.Constants.EventCode.ErrorInfo, (data) => {
        ...
        const LbcPeer = /** @class */ (function (_super) {
            ...
            LbcPeer.prototype.webRpcHandler = function (lbc) {
                ...
                return function (d) {
                    ...
                    let uriPath; let message; let data; let
                        resultCode;
                    if (d.errCode == 0) {
        ...
        var NameServerPeer = /** @class */ (function (_super) {
            ...
            function NameServerPeer(client, protocol, address, subprotocol) {
                const _this = _super.call(this, protocol, address, subprotocol, `${client.logger.getPrefix()} NameServer`) || this;
                _this.client = client;
                return _this;
            }
            // overrides
        ...
        var GamePeer = /** @class */ (function (_super) {
            ...
            GamePeer.prototype.raiseEvent = function (eventCode, data, options) {
                if (this.client.isJoinedToRoom()) {
                    ...
                    const params = [LoadBalancing.Constants.ParameterCode.Code, Photon.TypeExt.Byte(eventCode), LoadBalancing.Constants.ParameterCode.Data, data];
                    if (options) {
...
(function (Photon) {
    ...
    (function (LoadBalancing) {
        let Constants;
        (function (Constants) {
            const { LiteOpKey } = Photon.Lite.Constants;
            const { LiteOpCode } = Photon.Lite.Constants;
            const { LiteEventCode } = Photon.Lite.Constants;
            /**
            ...
            Channel.prototype.readProperties = function (props) {
                if (props) {
                    for (const p in props) {
                        if (props[p] == null) { this.properties[p] = undefined; } else { this.properties[p] = props[p]; }
                    }
    ...
    //            @borrows Photon.LoadBalancing.LoadBalancingClient#connectToNameServer // overrides with connectToNameServer with different parameters
            ...
            */
            ...
            ChatClient.prototype.initMasterPeer = function (mp) {
                ...
                // });
                mp.addEventListener(Chat.Constants.EventCode.ChatMessages, (data) => {
                ...
                mp.addEventListener(Chat.Constants.EventCode.PrivateMessage, (data) => {
                ...
                mp.addEventListener(Chat.Constants.EventCode.StatusUpdate, (data) => {
                    const sender = data.vals[Chat.Constants.ParameterCode.Sender];
                ...
                mp.addEventListener(Chat.Constants.EventCode.Subscribe, (data) => {
                ...
                mp.addEventListener(Chat.Constants.EventCode.Unsubscribe, (data) => {
                ...
                mp.addEventListener(Chat.Constants.EventCode.UserSubscribe, (data) => {
                ...
                mp.addEventListener(Chat.Constants.EventCode.UserUnsubscribe, (data) => {
                    ...
                    } else if (!ch.removeSubscriber(userId)) {
